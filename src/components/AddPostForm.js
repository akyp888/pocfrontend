...
const AddPostForm = () => {
    ...
    const [reqtype,setReqtype] = useState("post")
     ...
     const add = async(e) => {
        ...
        await axios.post("http://http://52.206.79.151/:8080/api/posts/", JSON.stringify(formData),{headers: {'Content-Type': 'application/json',}})
        e.preventDefault()
        }
        if(reqtype === "put"){
            console.log( JSON.stringify(formData))
           await axios.put("http://http://52.206.79.151/:8080/api/posts/", JSON.stringify(formData),{headers: {'Content-Type': 'application/json',}}).then(response => console.log(response))
        e.preventDefault()
